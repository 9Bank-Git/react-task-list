...
export default function TaskItems({ displayTasks, showActive, setShowActive, toggleCompleted, moveTaskUp, moveTaskDown, editedTask, openModalDialog }) {
  // const { displayTasks, showActive, setShowActive, toggleCompleted, moveTaskUp, moveTaskDown, editedTask, openModal } = props
  return (
    <div className='task-list flex flex-col'>
      <div className='relative flex'>
        <p className='flex items-center text-lg mt-2 mb-1'>Task List</p>
        <div className='absolute top-3.5 left-25 flex items-center absolute'>
          ...
          <span className='pl-1 text-sm text-gray-700'>Show only active</span>
        </div>
      </div>
  ...
  return (
    <li className='flex flex-col sm:flex-row px-2 py-3 gap-y-2 sm:gap-x-2 rounded shadow-sm bg-gray-100 items-center'>
      <div className='flex pl-2 gap-x-4 items-center'>
        ...
        <span className={`size-5 rounded-full font-semibold text-sm text-center content-center ${task.completed ? 'text-gray-100 bg-sky-200' : 'text-white bg-sky-500'}`}>
          {index + 1}
        </span>
      </div>
      {isEditing ? (
        ...
        <span className={`flex-1 font-light text-lg text-center sm:text-left break-all ${task.completed ? 'line-through text-gray-400' : 'text-gray-700'}`}>{task.text}</span>
      )
      }
      {isEditing ? (
        <div className='actions flex space-x-1'>
          <button onClick={() => moveTaskUp(index)} className='flex px-1.5 py-1 rounded text-gray-500 hover:bg-zinc-200 items-center'>
            <i className='material-icons' style={{fontSize: '18px'}}>arrow_upward</i>
          </button>
          <button onClick={() => moveTaskDown(index)} className='flex px-1.5 py-1 rounded text-gray-500 hover:bg-zinc-200 items-center'>
            <i className='material-icons' style={{fontSize: '18px'}}>arrow_downward</i>
          </button>
        ...
        <div className='actions flex space-x-2'>
          <button onClick={() => handleEdit(index)} className='flex px-1.5 py-1 rounded text-gray-500 hover:bg-zinc-200 items-center'>
            <i className='material-icons' style={{fontSize: '18px'}}>create</i>
          </button>
